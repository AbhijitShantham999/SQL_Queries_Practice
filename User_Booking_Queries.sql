...
-- 1.write a sql query as segment,total user count,user who booked flights in april 2022

select u.segment,count(distinct u.user_id),
		count(distinct case when month(booking_date) = 4 and Line_of_business = 'flight' then b.User_id end )as cnt
...
select user_id,Booking_date from 
(select *,rank()over(partition by User_id order by booking_date) as ranknum from booking_table) sq
where ranknum = 1 and Line_of_business = 'hotel';

-- 3.write a query  to calculate the days between first and last booking of each user 

select *,abs(datediff(first_booking_date,last_booking_date)) diff from
(select User_id,min(Booking_date) first_booking_date,max(Booking_date) last_booking_date from booking_table
group by user_id) sq;

-- 4.write a query to count the number of flight and hotel booking in each of the user segments for the year 2022

select distinct segment,Line_of_business,cnt_buss from
(select u.Usedr_i,segment,Line_of_business,count(Line_of_business)over(partition by segment,Line_of_business) as cnt_buss
from booking_table b join user_table u 
on b.user_id = u.user_id) as sq;

_or_

select segment,sum(case when Line_of_business = 'flight' then 1 else 0 end )as flight,
